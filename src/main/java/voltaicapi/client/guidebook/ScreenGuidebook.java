...
import voltaicapi.client.guidebook.utils.components.Chapter;
import voltaicapi.client.guidebook.utils.components.Module;
import voltaicapi.client.guidebook.utils.components.Page;
import voltaicapi.client.guidebook.utils.components.Page.ChapterPage;
import voltaicapi.client.guidebook.utils.components.Page.CoverPage;
import voltaicapi.client.guidebook.utils.components.Page.GraphicWrapper;
import voltaicapi.client.guidebook.utils.components.Page.ModulePage;
import voltaicapi.client.guidebook.utils.components.Page.TextWrapper;
import voltaicapi.client.guidebook.utils.pagedata.AbstractWrapperObject;
import voltaicapi.client.guidebook.utils.pagedata.OnClick;
import voltaicapi.client.guidebook.utils.pagedata.OnKeyPress;
import voltaicapi.client.guidebook.utils.pagedata.OnTooltip;
import voltaicapi.client.guidebook.utils.pagedata.graphics.AbstractGraphicWrapper;
import voltaicapi.client.guidebook.utils.pagedata.graphics.AbstractGraphicWrapper.GraphicTextDescriptor;
import voltaicapi.client.guidebook.utils.pagedata.graphics.ImageWrapperObject;
import voltaicapi.client.guidebook.utils.pagedata.text.TextWrapperObject;
...
import voltaicapi.prefab.screen.component.button.type.ButtonGuidebook;
import voltaicapi.prefab.screen.component.button.type.ButtonGuidebook.GuidebookButtonType;
import voltaicapi.prefab.screen.component.button.type.ButtonModuleSelector;
import voltaicapi.prefab.screen.component.button.type.ButtonSearchedText;
import voltaicapi.prefab.screen.component.button.type.ButtonSpecificPage;
import voltaicapi.prefab.screen.component.editbox.type.EditBoxSpecificPage;
import voltaicapi.prefab.screen.component.types.ScreenComponentGuidebookArrow;
import voltaicapi.prefab.screen.component.types.ScreenComponentGuidebookArrow.ArrowTextures;
...
public class ScreenGuidebook extends GenericScreen<ContainerGuidebook> {
	...
	private static final ResourceLocation PAGE_TEXTURE_LEFT = VoltaicAPI.rl("textures/screen/guidebook/resources/guidebookpageleft.png");
	private static final ResourceLocation PAGE_TEXTURE_RIGHT = VoltaicAPI.rl("textures/screen/guidebook/resources/guidebookpageright.png");
	...
	private CoverPage getCoverPage() {
		...
		List<FormattedText> split = font.getSplitter().splitLines(ModularElectricityTextUtils.guidebook("title").withStyle(ChatFormatting.BOLD), TEXT_WIDTH, Style.EMPTY);
		...
		page.graphics.add(new GraphicWrapper(TEXT_START_X, y, new ImageWrapperObject(xOffset, 0, 0, 0, 74, 100, 74, 100, VoltaicAPI.rl("textures/screen/guidebook/resources/guidebookcover.png")), null, null, null));

		y += 105;

		split = font.getSplitter().splitLines(ModularElectricityTextUtils.guidebook("titlequote"), TEXT_WIDTH, Style.EMPTY);
	...
	private CoverPage getSearchPageLeft() {
		...
		List<FormattedText> split = font.getSplitter().splitLines(ModularElectricityTextUtils.guidebook("searchparameters").withStyle(ChatFormatting.BOLD), TEXT_WIDTH, Style.EMPTY);
		...
		page.text.add(new TextWrapper(TEXT_START_X + 15, 165, ModularElectricityTextUtils.guidebook("casesensitive"), TextWrapperObject.DEFAULT_COLOR, false, null, null, null));
		caseSensitive = new ButtonModuleSelector(70, 165, nextPageNumber, false);

		BUTTONS.add(new ButtonSpecificPage(-71, 180, 75, 20, nextPageNumber).setLabel(ModularElectricityTextUtils.guidebook("selectall")).setOnPress(button -> {
			for (ButtonModuleSelector selector : MODULE_PARAMETERS) {
				selector.setSelected(true);
			}
		}));

		BUTTONS.add(new ButtonSpecificPage(8, 180, 75, 20, nextPageNumber).setLabel(ModularElectricityTextUtils.guidebook("selectnone")).setOnPress(button -> {
			for (ButtonModuleSelector selector : MODULE_PARAMETERS) {
