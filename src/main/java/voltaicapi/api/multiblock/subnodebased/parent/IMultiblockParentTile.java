package voltaicapi.api.multiblock.subnodebased.parent;

import org.jetbrains.annotations.Nullable;

import voltaicapi.api.electricity.ICapabilityElectrodynamic;
import voltaicapi.api.gas.IGasHandler;
import voltaicapi.api.multiblock.subnodebased.Subnode;
import voltaicapi.api.multiblock.subnodebased.child.IMultiblockChildBlock;
        ...
        for (int i = 0; i < subnodes.length; i++) {
            ...
            if (update) {
                if (subnodeState.canBeReplaced()) {
                    world.setBlockAndUpdate(offset, VoltaicAPIBlocks.BLOCK_MULTISUBNODE.get().defaultBlockState());
                }
