...
public class CapabilityLocationStorage implements ILocationStorage, INBTSerializable<CompoundTag> {
	...
	public CompoundTag serializeNBT(HolderLookup.Provider provider) {
		if (VoltaicAPICapabilities.CAPABILITY_LOCATIONSTORAGE_ITEM != null) {
			CompoundTag nbt = new CompoundTag();
			nbt.putInt("size", locations.size());
			for (int i = 0; i < locations.size(); i++) {
				locations.get(i).writeToNBT(nbt, VoltaicAPICapabilities.LOCATION_KEY + i);
			}
			return nbt;
		}
	...
	public void deserializeNBT(HolderLookup.Provider provider, CompoundTag nbt) {
		if (VoltaicAPICapabilities.CAPABILITY_LOCATIONSTORAGE_ITEM != null) {
			...
			for (int i = 0; i < nbt.getInt("size"); i++) {
				locations.add(Location.readFromNBT(nbt, VoltaicAPICapabilities.LOCATION_KEY + i));
			}
		}
	}
