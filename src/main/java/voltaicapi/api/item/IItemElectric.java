...
import voltaicapi.prefab.item.ElectricItemProperties;
...
public interface IItemElectric {
    ...
    default double getJoulesStored(ItemStack stack) {
        return stack.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData(this)).joulesStored;
    }

    public static void setEnergyStored(ItemStack stack, double amount) {
        ElectricItemData data = stack.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) stack.getItem()));
        data.joulesStored = amount;
        stack.set(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, data);
    }

    default double getMaximumCapacity(ItemStack item) {
        return item.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) item.getItem())).maxJoules;
    }

    static void setMaximumCapacity(ItemStack item, double amt) {
        ElectricItemData data = item.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) item.getItem()));
        data.maxJoules = amt;
        item.set(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, data);
    }

    default double getReceiveLimit(ItemStack item) {
        return item.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) item.getItem())).receiveCap;
    }
    ...
    default double getExtractLimit(ItemStack item) {
        return item.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) item.getItem())).extractCap;
    }
            ...
            if (!playerItem.isEmpty() && playerItem.getItem() instanceof IItemElectric electric && electric.isEnergyStorageOnly() && electric.getJoulesStored(playerItem) > 0 && electric.getElectricProperties().extract.getVoltage() == electricItem.getElectricProperties().extract.getVoltage() && electric.getElectricProperties().receive.getVoltage() == electricItem.getElectricProperties().receive.getVoltage()) {
                ...
                for (int j = 0; j < inv.items.size(); j++) {
                    ...
                    if (item.isEmpty()) {
                        ...
                        player.level().playSound(null, player.getOnPos(), VoltaicAPISounds.SOUND_BATTERY_SWAP.get(), SoundSource.PLAYERS, 0.25F, 1.0F);
                        return;
                    }
                }
                return;
    ...
    static boolean overrideOtherStackedOnMe(ItemStack stack, ItemStack other, Slot slot, ClickAction action, Player player, SlotAccess access) {
        ...
        player.level().playLocalSound(player.getX(), player.getY(), player.getZ(), VoltaicAPISounds.SOUND_BATTERY_SWAP.get(), SoundSource.PLAYERS, 0.25F, 1.0F, false);
    ...
    default ItemStack getCurrentBattery(ItemStack tool) {
        ...
        ElectricItemData data = tool.getOrDefault(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, defaultData((IItemElectric) tool.getItem()));
    ...
    default void setCurrentBattery(ItemStack tool, ItemStack battery) {
        ...
        tool.set(VoltaicAPIDataComponentTypes.ELECTRIC_ITEM_DATA, data);
    }
    ...
    static void addBatteryTooltip(ItemStack stack, Item.TooltipContext context, List<Component> tooltip) {
        tooltip.add(ModularElectricityTextUtils.tooltip("currbattery", ((IItemElectric) stack.getItem()).getCurrentBattery(stack).getDisplayName().copy().withStyle(ChatFormatting.GRAY)).withStyle(ChatFormatting.DARK_GRAY));
    }
