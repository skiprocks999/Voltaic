...
import voltaicapi.api.radiation.util.*;
...
public class RadiationManager implements IRadiationManager {
    ...
    public List<SimpleRadiationSource> getPermanentSources(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES).values());
    }

    @Override
    public List<TemporaryRadiationSource> getTemporarySources(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES).values());
    }

    @Override
    public List<FadingRadiationSource> getFadingSources(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES).values());
    }

    @Override
    public List<BlockPos> getPermanentLocations(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES).keySet());
    }

    @Override
    public List<BlockPos> getTemporaryLocations(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES).keySet());
    }

    @Override
    public List<BlockPos> getFadingLocations(Level world) {
        return new ArrayList<>(world.getData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES).keySet());
    }

    @Override
    public void addRadiationSource(SimpleRadiationSource source, Level world) {
        if (source.isTemporary()) {
            HashMap<BlockPos, TemporaryRadiationSource> sources = world.getData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES);
            ...
            world.setData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES, sources);
        } else {
            HashMap<BlockPos, SimpleRadiationSource> sources = world.getData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES);
            ...
            world.setData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES, sources);
        }
    }

    @Override
    public int getReachOfSource(Level world, BlockPos pos) {
        return Math.max(world.getData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES).getOrDefault(pos, TemporaryRadiationSource.NONE).distance, Math.max(world.getData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES).getOrDefault(pos, SimpleRadiationSource.NONE).getDistanceSpread(), world.getData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES).getOrDefault(pos, FadingRadiationSource.NONE).distance));
    }

    @Override
    public void setDisipation(double radiationDisipation, Level world) {
        world.setData(VoltaicAPIAttachmentTypes.DEFAULT_DISSIPATION, radiationDisipation);
    }

    @Override
    public void setLocalizedDisipation(double disipation, BlockPosVolume area, Level world) {
        HashMap<BlockPosVolume, Double> values = world.getData(VoltaicAPIAttachmentTypes.LOCALIZED_DISSIPATIONS);
        values.put(area, disipation + values.getOrDefault(area, 0.0));
        world.setData(VoltaicAPIAttachmentTypes.LOCALIZED_DISSIPATIONS, values);
    }
    ...
    public boolean removeRadiationSource(BlockPos pos, boolean shouldLeaveFadingSource, Level world) {
        ...
        if (shouldLeaveFadingSource) {
            HashMap<BlockPos, FadingRadiationSource> fadingSources = world.getData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES);
            ...
            world.setData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES, fadingSources);
        }
        return true;
    }
    ...
    public void wipeAllSources(Level level) {
        level.removeData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES);
        level.removeData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES);
        level.removeData(VoltaicAPIAttachmentTypes.FADING_RADIATION_SOURCES);
    }
    ...
    public void tick(Level world) {
        ...
        for (Map.Entry<BlockPos, SimpleRadiationSource> entry : world.getData(VoltaicAPIAttachmentTypes.PERMANENT_RADIATION_SOURCES).entrySet()) {
            ...
            for (LivingEntity entity : world.getEntitiesOfClass(LivingEntity.class, new AABB(position.getX() - permanentSource.distance(), position.getY() - permanentSource.distance(), position.getZ() - permanentSource.distance(), position.getX() + permanentSource.distance() + 1, position.getY() + permanentSource.distance() + 1, position.getZ() + permanentSource.distance() + 1))) {
                IRadiationRecipient capability = entity.getCapability(VoltaicAPICapabilities.CAPABILITY_RADIATIONRECIPIENT);
                if (capability == null) {
                    continue;
                }
        ...
        HashMap<BlockPos, TemporaryRadiationSource> temporarySources = world.getData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES);
        ...
        if (changed) {
            world.setData(VoltaicAPIAttachmentTypes.TEMPORARY_RADIATION_SOURCES, temporarySources);
        }
        ...
        double defaultRadiationDisipation = world.getData(VoltaicAPIAttachmentTypes.DEFAULT_DISSIPATION);

        HashMap<BlockPosVolume, Double> localizedDissipations = world.getData(VoltaicAPIAttachmentTypes.LOCALIZED_DISSIPATIONS);
