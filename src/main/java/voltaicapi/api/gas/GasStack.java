...
public class GasStack {
    ...
    private static final Codec<GasStack> INTERNAL_CODEC = RecordCodecBuilder.create(instance ->
                    ...
                    instance.group(
                            //
                            VoltaicAPIGases.GAS_REGISTRY.holderByNameCodec().fieldOf("gas").forGetter(GasStack::getGasHolder),
                            //
...
//        
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, GasStack> STREAM_CODEC = new StreamCodec<>() {
        ...
        private static final StreamCodec<RegistryFriendlyByteBuf, Holder<Gas>> GAS_STREAM_CODEC = ByteBufCodecs.holderRegistry(VoltaicAPIGases.GAS_REGISTRY_KEY);
    ...
    public Gas getGas() {
        return isEmpty() ? VoltaicAPIGases.EMPTY.get() : gas;
    }
    ...
    public boolean isEmpty() {
        return this == EMPTY || this.gas == VoltaicAPIGases.EMPTY.value() || this.amount <= 0;
    }
    ...
    public CompoundTag writeToNbt() {
        ...
        if (isEmpty()) {
            ...
            tag.putString("name", VoltaicAPIGases.GAS_REGISTRY.getKey(getGas()).toString());
    ...
    public static GasStack readFromNbt(CompoundTag tag) {
        ...
        if (amount <= 0) {
            ...
            Gas gas = VoltaicAPIGases.GAS_REGISTRY.get(ResourceLocation.parse(tag.getString("name")));
