...
public class PropertyManager {
	...
	public void loadDataFromClient(int index, CompoundTag data) {
		if(index >= properties.size()) {
			VoltaicAPI.LOGGER.error("The tile at " + owner.getBlockPos() + " has a differently sized property list than what was declared by the packet");
			return;
		}
		Property<?> prop = properties.get(index);
		if(owner == null) {
			VoltaicAPI.LOGGER.info("The property " + prop.getName() + " is sending data to a null tile");
			return;
		}
		if(owner.getLevel() == null){
			VoltaicAPI.LOGGER.info("The property " + prop.getName() + " that sent data to the tile at " + owner.getBlockPos() + " encountered a null level. The data was not loaded");
			return;
		}
