...
public class ScreenComponentGasTemperature extends ScreenComponentGuiTab {
	...
	private List<? extends FormattedCharSequence> getMaxPressureInfo() {
		...
		for (PropertyGasTank tank : handler.getInputTanks()) {
			...
			tooltips.add(ModularElectricityTextUtils.tooltip("tankmaxin", index, ChatFormatter.getChatDisplayShort(tank.getMaxTemperature(), DisplayUnits.TEMPERATURE_KELVIN)).withStyle(ChatFormatting.GRAY).getVisualOrderText());
		...
		for (PropertyGasTank tank : handler.getOutputTanks()) {
			...
			tooltips.add(ModularElectricityTextUtils.tooltip("tankmaxout", index, ChatFormatter.getChatDisplayShort(tank.getMaxTemperature(), DisplayUnits.TEMPERATURE_KELVIN)).withStyle(ChatFormatting.DARK_GRAY).getVisualOrderText());
