...
public class ComponentGasHandlerMulti implements IComponentGasHandler {
    ...
    public void onLoad() {
        ...
        if (recipeType != null) {
            List<RecipeHolder<ModularElectricityRecipe>> recipes = ModularElectricityRecipe.findRecipesbyType(recipeType, holder.getLevel());
            ...
            for (RecipeHolder<ModularElectricityRecipe> iRecipe : recipes) {
            ...
            if (validInputGasTags != null) {
                for (TagKey<Gas> tag : validInputGasTags) {
                    VoltaicAPIGases.GAS_REGISTRY.getTag(tag).get().stream().forEach(holder -> {
