...
import voltaicapi.common.network.utils.FluidUtilities;
import voltaicapi.common.network.utils.GasUtilities;
...
import voltaicapi.common.recipe.categories.fluid2gas.Fluid2GasRecipe;
import voltaicapi.common.recipe.categories.fluid2item.Fluid2ItemRecipe;
import voltaicapi.common.recipe.categories.fluiditem2fluid.FluidItem2FluidRecipe;
import voltaicapi.common.recipe.categories.fluiditem2gas.FluidItem2GasRecipe;
import voltaicapi.common.recipe.categories.fluiditem2item.FluidItem2ItemRecipe;
import voltaicapi.common.recipe.categories.item2fluid.Item2FluidRecipe;
import voltaicapi.common.recipe.categories.item2item.Item2ItemRecipe;
...
import voltaicapi.prefab.utilities.ItemUtils;
...
public class ComponentProcessor implements IComponent {
    ...
    private List<RecipeHolder<ModularElectricityRecipe>> cachedRecipes = new ArrayList<>();
    private ModularElectricityRecipe recipe;
    ...
    public ModularElectricityRecipe getRecipe() {
        return recipe;
    }

    public void setRecipe(ModularElectricityRecipe recipe) {
        this.recipe = recipe;
    }
        ...
        for (ItemStack stack : inv.getUpgradeContents()) {
            ...
            if (!stack.isEmpty()) {
                ...
                if (upgrade.subtype == SubtypeItemUpgrade.experience) {
                    stack.set(VoltaicAPIDataComponentTypes.XP, stack.getOrDefault(VoltaicAPIDataComponentTypes.XP, 0.0) + getStoredXp());
                    setStoredXp(0);
                    break;
                }
            }
    ...
    @Nullable
    public ModularElectricityRecipe getRecipe(ComponentProcessor pr, RecipeType<?> typeIn) {
        if (cachedRecipes.isEmpty()) {
            cachedRecipes = ModularElectricityRecipe.findRecipesbyType((RecipeType<ModularElectricityRecipe>) typeIn, pr.getHolder().getLevel());
        }
        return ModularElectricityRecipe.getRecipe(pr, cachedRecipes);
    }
