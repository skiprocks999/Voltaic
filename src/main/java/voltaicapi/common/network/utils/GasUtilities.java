package voltaicapi.common.network.utils;
...
import voltaicapi.prefab.utilities.BlockEntityUtils;
...
public class GasUtilities {
    ...
    public static boolean isGasReciever(BlockEntity acceptor, Direction dir) {
        return acceptor != null && acceptor.getLevel().getCapability(VoltaicAPICapabilities.CAPABILITY_GASHANDLER_BLOCK, acceptor.getBlockPos(), acceptor.getBlockState(), acceptor, dir) != null;
    }
    ...
    public static int recieveGas(BlockEntity reciever, Direction dir, GasStack gas, GasAction action) {
        ...
        IGasHandler handler = reciever.getLevel().getCapability(VoltaicAPICapabilities.CAPABILITY_GASHANDLER_BLOCK, reciever.getBlockPos(), reciever.getBlockState(), reciever, dir);
    ...
    public static void outputToPipe(GenericTile tile, GasTank[] tanks, Direction... outputDirections) {
        ...
        for (Direction relative : outputDirections) {
            ...
            IGasHandler handler = faceTile.getLevel().getCapability(VoltaicAPICapabilities.CAPABILITY_GASHANDLER_BLOCK, faceTile.getBlockPos(), faceTile.getBlockState(), faceTile, direction.getOpposite());
