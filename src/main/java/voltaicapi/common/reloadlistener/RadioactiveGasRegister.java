...
import voltaicapi.common.packet.types.client.PacketSetClientRadioactiveGases;
...
public class RadioactiveGasRegister extends SimplePreparableReloadListener<JsonObject> {
    ...
    protected void apply(JsonObject json, ResourceManager manager, ProfilerFiller profiler) {
        ...
        json.entrySet().forEach(set -> {
            ...
            if (key.contains("#")) {
                ...
                radioactiveGasMap.put(VoltaicAPIGases.GAS_REGISTRY.get(ResourceLocation.parse(key)), value);
    ...
    public void generateTagValues() {
        ...
        tags.forEach((tag, value) -> {
            VoltaicAPIGases.GAS_REGISTRY.getTag(tag).get().forEach(fluid -> {
