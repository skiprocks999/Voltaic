...
import voltaicapi.datagen.utils.client.model.SlaveNodeModelBuilder;
import voltaicapi.datagen.utils.client.model.WireModelBuilder;
...
public abstract class BaseBlockstateProvider extends BlockStateProvider {
    ...
    public ItemModelBuilder glassBlock(Block block, ResourceLocation texture, boolean registerItem) {
        return simpleBlockCustomRenderType(block, texture, VoltaicAPI.vanillarl("cutout"), registerItem);
    }
    ...
    public void wire(Block block, ModelFile none, ModelFile side, boolean registerItem) {
        ModelFile model = models().withExistingParent(name(block), VoltaicAPI.vanillarl("cube")).customLoader(WireModelBuilder::begin).models(none, side, side).end().renderType(VoltaicAPI.vanillarl("cutout"));
    ...
    public void slaveNode(Block block, String particleTexture) {
        BlockModelBuilder builder = models().getBuilder(name(block)).customLoader(SlaveNodeModelBuilder::begin).model(existingBlock(VoltaicAPIBlocks.BLOCK_MULTISUBNODE.get())).end().texture("particle", modLoc(particleTexture)).renderType("cutout");
        getVariantBuilder(block).partialState().setModels(new ConfiguredModel(builder));
    }
